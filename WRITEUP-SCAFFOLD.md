...
- **The proof that would not close (SS §O).** (1) I believed the machine reached a fixed point, so a bounded proof would be a complete one. (2) Cost: k-induction ran 23 minutes without converging; the quiescence script was written to establish the fixed point. (3) Low point: nine flops can still move, and neither success-latch flop is among them from an arbitrary finished state. (4) NO correction — this one stays open, and the honest beat 4 is naming the two mechanisms (differently encoded state spaces; unreachable states) rather than resolving.
...
- **Cell-type counts, use exactly one framing:** 69 distinct sky130 cell types are instantiated on the die; 66 of those have logic (69 minus `decap`, `diode`, `tapvpwrvgnd`); 67 functional model *files* were fetched, fewer than 69 because drive strengths share a base model (`clkbuf_4/8/16` all map to `clkbuf`). Never quote two of these in one document without saying which is which.
...
- **STRUGGLE 1 of 5 (chosen).** Form: narrative dead end. The RTL asserted the verdict immediately; the silicon registers it one cycle later. Every simulation against the known-good stream had passed. RESOLVED 2026-08-12, and the claim is stronger than first written: the solution grid is the counterexample **necessarily**, not by luck. Proof in `cycle124.py forced` — pin bit i off-solution and leave the other 120 free, once per position; all 121 come back UNSAT, while the control with the whole grid pinned fails as it must. So no counterexample differs from the solution at any position. Mechanism, one clause: the two designs compute the same verdict and differ only in when they latch it, every output is gated by `success`, and exactly one grid is ever accepted — so the miter can only observe a difference on the single accepting run. Keep the unroll-depth bound attached: this is at depth 124.

- **Heading:** should not be cute. Name the cycle.
- **Claim to land:** every simulation against the known-good stimulus passed, and the reconstruction was still wrong; the miter found it at exactly cycle 124, and the counterexample it must return is the solution grid itself — forced, not coincidental.
- **Facts/numbers:** silicon registers the verdict **one cycle after** the last bit; the RTL asserted it combinationally. Reproduce with `cycle124.py`: input pinned to the solution grid, equivalent at unroll depth 123, counterexample at 124. Uniqueness with `cycle124.py forced`: 121 runs, bit i pinned off-solution and the other 120 free, all UNSAT at ~3.09M variables each; control with the whole grid pinned fails. Say **forced**, cite the 121 cases, and keep "at this unroll depth" attached — do not drop the bound.
- **The mechanism is one clause and it is what makes this publishable:** the two designs compute the same verdict and differ only in when they latch it; every output is gated by `success`; exactly one grid is ever accepted; therefore the only run on which a difference is observable is the accepting one. Tie to their own puzzle instruction: "Don't forget to toggle `rst_n` before each input attempt" — the same one-cycle protocol subtlety, from the other side.
...
- **STRUGGLE 2 of 5 (chosen).** Form: failed assumption. The first attempt assumed the bytes were sitting in the gates. They are not, and the masks `4d ad fb 83 … 8f, plus 6a at the saturated index` are what proves it. The wrong assumption is what makes the finding land.

- **Claim to land:** `O = permute(LFSR) XOR mask(index)`; the mask bytes sitting in the gates are `4d ad fb 83 13 79 1c b5 79 63 c7 68 93 f5 8f`, plus a sixteenth entry `6a` at the saturated index — not text — and the plaintext exists only once the LFSR runs from its reset seed.
...
- **Must NOT:** overstate the held-out framing. If the hint image had been seen before the floorplan was finalised, say so precisely; the 207/208 number survives either way, the epistemics don't.

---

### §M — Back to hardware (≈300 w)

- **Claim to land:** the model was written out twice, independently, and both were machine-proved against the silicon; then re-synthesized onto a Liberty built from the die itself, where the two reconstructions differ substantially through the identical flow (451 vs 363 cells).
- **Facts/numbers:** Verilog 169 lines, Hardcaml 109 lines; both proved by bounded miter — **145 cycles under the operating protocol, 122 with every input free**; Verilog run: 3,100,406 vars / 8,531,120 clauses, 145 time steps; Hardcaml run: 1,831,366 vars / 4,842,030 clauses. Quote var/clause counts, never wall-clock — those reproduce exactly, timings do not. Liberty: cell *functions* from the PDK, cell **areas measured off `puzzle.gds`**. Area comparison: die **728 cells / 10,758 µm²** vs Verilog **451 / 6,816** vs Hardcaml **363 / 5,892** (through the identical flow, `synth.ys` and `synth_hardcaml.ys`; the two reconstructions do NOT converge in size, and any earlier 'within one cell' claim was an artifact of two different flows). Tiny Tapeout: packaged and tested, `uio_oe = 00000001`, `success = 1`, message 15 bytes, `$finish at 1446000`; **43% of a 1×1 tile**.
...
2. **The most-tested input is the *only* counterexample, and necessarily so.** Every simulation against the known-good stimulus passed; the miter failed at cycle 124, and the single input that can expose the bug is the solution grid itself — proved by 121 UNSAT runs, one per pinned-off position, not observed by luck. One cycle of latency, invisible to the test that mattered. This is simultaneously the debugging-honesty datum they explicitly reward and a live instance of their current editorial thesis about verification burden — and it earns that reading without the document ever having to argue for it.
...
| "We used their hint as a test, not an input" | Only true if the hint image was genuinely not consulted while deriving the floorplan. If it was seen first, the honest version is that the correspondence was *quantified* (207/208, 12/12, 0 foreign flops) rather than that it was held out. The number survives; the epistemic framing must match what actually happened. |
